...
strTypG creaElemStr(int w, int h, unsigned char cor, int rc, int cc){
	...
	// Se as cordenadas do centro forem maiores q a dimensao do Elemento Estruturante ou menores q 0 ele da sai da funcao
	...
	// Preenche o elemento generica aqui, 1 para pertencer e 0 para nao pertencer
	for (i = 0; i < w; ++i){
		for (j = 0; j < h; ++j){
			scanf("%d",&matriz[i][j]);
			//matriz[i][j] = 1;
	...
	// Imprime a matriz de presenca com destaque para o valor do centro q ficara entre chaves []
...
Image * DilGrayScl(Image *entrada, strTypG *SrtEl){	
	...
	for(y = 0; y < imgGetHeight(entrada); y++){
		for(x = 0; x < imgGetWidth(entrada); x++){
			...
			// Confere se o centro do Elemento Estruturante nao saiu da imagem
			if(  ( (y + SrtEl->rc) < 0 || (y + SrtEl->rc) > imgGetHeight(entrada) ) || 
				(  (x + SrtEl->cc) < 0 || (x + SrtEl->cc) > imgGetWidth(entrada))
				)
				continue;

			// Confere se a cor do Elemento Estruturante central é a mesma da imagem
			if( SrtEl->matOper[(SrtEl->rc * SrtEl->w) + SrtEl->cc].cor & imgGetPixelRed(entrada,x,y)){
				...
				for( i = 0 ; i < (SrtEl->w * SrtEl->h) ; i++) {
					...
					// Confere se o restante do Elemento Estruturante nao saiu da imagem
					if ((y1 >= 0 && x1 >= 0) && 
